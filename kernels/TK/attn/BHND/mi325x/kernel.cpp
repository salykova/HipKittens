...
template<int D> constexpr size_t ROWS = 32; // height of each worker tile (rows)
...
template<int D> struct attn_globals { 
    ...
    dim3 grid() { return dim3((ATTN_N + qkvo_tile<ATTN_D>::rows*NUM_WORKERS - 1) / (qkvo_tile<ATTN_D>::rows*NUM_WORKERS), ATTN_H, ATTN_B); }
    dim3 block() { return dim3(32*NUM_WORKERS); }
...
__global__ void attend_ker(const attn_globals<D> g) {
    ...
    else if constexpr(D == 128) mul(q_reg, q_reg, __float2bfloat16(0.08838834764f * 1.44269504089f));
    ...
    //         right_fill(att_block, att_block, start_fill, base_types::constants<float>::neg_infty());
            ...
            // copy(max_vec, max<axis::COL>(att_block, max_vec)); 
            // sub(att_block, att_block, max_vec);
