...
template<int D> struct attn_globals { 
    gl<bf16, -1, -1, -1, D> Qg, Kg, Vg, Ag, Og; 
...
__global__ void attend_ker(const attn_globals<D> g) {
    ...
    for (int j = 0; j < num_tiles; j++) {
        ...
        swap_layout(att_block_row, att_block);
        ...
        swap_layout(o_reg_next_row, o_reg_next);
...
PYBIND11_MODULE(tk_kernel_row, m) {
    m.doc() = "tk_kernel_row python module";
    py::bind_function<dispatch_micro<ATTN_D>>(m, "dispatch_micro", &attn_globals<ATTN_D>::Qg, &attn_globals<ATTN_D>::Kg, &attn_globals<ATTN_D>::Vg, &attn_globals<ATTN_D>::Ag, &attn_globals<ATTN_D>::Og);
}
