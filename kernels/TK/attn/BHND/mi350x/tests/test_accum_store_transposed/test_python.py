...
A = torch.randn(BATCH_SIZE, HEAD_SIZE, ROWS, COLS, dtype=torch.bfloat16, device='cuda') / 10.0  
C = torch.zeros(BATCH_SIZE, HEAD_SIZE, ROWS, COLS, dtype=torch.bfloat16, device='cuda')

tk_kernel.dispatch_micro(A, C)

# C_ref = torch.matmul(A, A.t()).float()
C_ref = A.float().contiguous()
