...
void micro_tk(const micro_globals g) {
    ...
    rt_fl<BLOCK_SIZE, BLOCK_SIZE, accum_col_l> C_accum1;
    rt_fl<BLOCK_SIZE, BLOCK_SIZE, accum_col_l> C_accum2;
    ...
    for (int tile = 0; tile < num_tiles-1; ++tile, tic ^= 1, toc ^= 1) {
        ...
        if (is_producer) {
            ...
            load(b1, subtile_inplace<BLOCK_SIZE, DOT_SLICE>(Bs[tic][local_warp_id+4], {0,1}), lane_ofs);
            asm volatile("s_waitcnt lgkmcnt(0)");
            __builtin_amdgcn_s_setprio(1);
            mma_ABt(C_accum1, a0, b0, C_accum1);
            mma_ABt(C_accum2, a0, b1, C_accum2);
