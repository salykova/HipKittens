...
void micro_tk(const micro_globals g) {
    ...
    st_bf<BLOCK_SIZE_MN, K_TILE, ducks::st_layout::row> (&As)[2][M_BLOCK] = al.allocate<st_bf<BLOCK_SIZE_MN, K_TILE, ducks::st_layout::row>, 2, M_BLOCK>();
    st_bf<BLOCK_SIZE_MN, K_TILE, ducks::st_layout::row> (&Bs)[2][N_BLOCK] = al.allocate<st_bf<BLOCK_SIZE_MN, K_TILE, ducks::st_layout::row>, 2, N_BLOCK>();
    ...
    using T = typename st_bf<BLOCK_SIZE_MN, K_TILE>::dtype;
    ...
    for (int tile = 0; tile < num_tiles-1; ++tile, tic ^= 1, toc ^= 1) {
        ...
        if (is_producer) {
            ...
            A_slice a0;
            B_slice b0;
            load(a0, subtile_inplace<BLOCK_SIZE_MN, DOT_SLICE>(As[tic][consumer_idx], {0,0}), lane_ofs);
            load(b0, subtile_inplace<BLOCK_SIZE_MN, DOT_SLICE>(Bs[tic][local_warp_id], {0,0}), lane_ofs);
            asm volatile("s_waitcnt lgkmcnt(0)");
            __builtin_amdgcn_s_setprio(1);
            mma_ABt(C_accum, a0, b0, C_accum);
            __builtin_amdgcn_s_setprio(0);

            load(a0, subtile_inplace<BLOCK_SIZE_MN, DOT_SLICE>(As[tic][consumer_idx], {0,1}), lane_ofs);
            load(b0, subtile_inplace<BLOCK_SIZE_MN, DOT_SLICE>(Bs[tic][local_warp_id], {0,1}), lane_ofs);
            asm volatile("s_waitcnt lgkmcnt(0)");
            __builtin_amdgcn_s_setprio(1);
            mma_ABt(C_accum, a0, b0, C_accum);
            __builtin_amdgcn_s_setprio(0);

            load(a0, subtile_inplace<BLOCK_SIZE_MN, DOT_SLICE>(As[tic][consumer_idx], {0,2}), lane_ofs);
            load(b0, subtile_inplace<BLOCK_SIZE_MN, DOT_SLICE>(Bs[tic][local_warp_id], {0,2}), lane_ofs);
            asm volatile("s_waitcnt lgkmcnt(0)");
            __builtin_amdgcn_s_setprio(1);
            mma_ABt(C_accum, a0, b0, C_accum);
            __builtin_amdgcn_s_setprio(0);
