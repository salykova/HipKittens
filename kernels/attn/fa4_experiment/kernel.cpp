...
constexpr float RESCALE_THRESHOLD = 4.0f;  // Only rescale if scale factor < 2^-4
constexpr int E2E_FREQ = 16;  // Use polynomial every 16th element
constexpr int E2E_RES = 4;    // Reserve last 4 elements for SFU
constexpr int E2E_FRG_LIMIT = 1;  // Don't use polynomial on last fragment

// Modified exp2 function with mixed strategy
template<typename T, ducks::rt_layout::all layout, ducks::rt_shape::all shape>
__device__ inline void exp2_optimized(rt_base<T, layout, shape> &dst, 
                                      const rt_base<T, layout, shape> &src,
                                      int tile_idx, int num_tiles) {
    static_assert(std::is_same_v<shape, rt_32x32_s>, "Only 32x32 tiles supported");
    
    // Determine if we should use E2E for this tile
    bool use_e2e = (tile_idx < num_tiles - E2E_FRG_LIMIT) && (ATTN_D <= 128);
    
    #pragma unroll
    for(int k = 0; k < dst.packed_per_thread; k++) {
        bool use_poly = use_e2e && ((k % E2E_FREQ) >= (E2E_FREQ - E2E_RES));
        
        // Handle packed float2 data
        auto packed_val = src.data[k];
        
        if (1) {
            // Process both elements of the float2
            #pragma unroll
            for(int elem = 0; elem < 2; elem++) {
                float x = (elem == 0) ? packed_val.x : packed_val.y;
                float floor_x = floorf(x);
                float frac = x - floor_x;
                
                // Cubic approximation coefficients
                float poly = fma(0.0554906021f, frac, 0.2402265069f);
                poly = fma(poly, frac, 0.6931471805f);
                poly = fma(poly, frac, 1.0f);
                
                float result = ldexpf(poly, (int)floor_x);
                if (elem == 0) {
                    dst.data[k].x = result;
                } else {
                    dst.data[k].y = result;
            ...
            // Hardware SFU - apply exp2 to both components
...
template<typename T>
__device__ inline float get_max_diff(const T& vec_prev, const T& vec_new) {
    float max_diff = 0.0f;
    #pragma unroll
...
__global__ void attend_ker(const attn_globals<D> g) {
    ...
    exp2_optimized(att_block[0].tiles[0][0], att_block[0].tiles[0][0], 0, num_tiles);
    __builtin_amdgcn_sched_barrier(0);
    ...
    for (int j = 3; j < num_tiles - 1; j += 2) {
        ...
        exp2_optimized(att_block[0].tiles[1][0], att_block[0].tiles[1][0], j - 1, num_tiles);
        ...
        if (max_scale_diff * TEMPERATURE_SCALE >= -RESCALE_THRESHOLD) {
            // Skip rescaling - difference is small enough
            copy(max_vec, max_vec_prev);  // Keep old max
            // No scaling of o_reg needed
            ...
            exp2(scale_vec, scale_vec);
            sub_col(att_block[1], att_block[1], max_vec);
            mul_col(o_reg, o_reg, scale_vec);  // Only scale when needed
        }
        // Use optimized exp2
        exp2_optimized(att_block[1].tiles[0][0], att_block[1].tiles[0][0], j - 1, num_tiles);
        ...
        exp2_optimized(att_block[1].tiles[1][0], att_block[1].tiles[1][0], j, num_tiles);
        mul(norm_vec, norm_vec, scale_vec);
        ...
        // Use optimized exp2
        exp2_optimized(att_block[0].tiles[0][0], att_block[0].tiles[0][0], j, num_tiles);
        __builtin_amdgcn_s_setprio(0);
        __builtin_amdgcn_sched_barrier(0);
        __builtin_amdgcn_s_barrier();
        __builtin_amdgcn_sched_barrier(0);
    ...
    exp2_optimized(att_block[0].tiles[1][0], att_block[0].tiles[1][0], num_tiles - 3, num_tiles);
    mul(norm_vec, norm_vec, scale_vec);
    ...
    col_max(max_vec, att_block[1], max_vec_prev);
    float max_scale_diff_ep2 = get_max_diff(max_vec_prev, max_vec);
    if (max_scale_diff_ep2 * TEMPERATURE_SCALE >= -RESCALE_THRESHOLD) {
    ...
    exp2_optimized(att_block[1].tiles[0][0], att_block[1].tiles[0][0], num_tiles - 3, num_tiles);
    ...
    exp2_optimized(att_block[1].tiles[1][0], att_block[1].tiles[1][0], num_tiles - 3, num_tiles);
    mul(norm_vec, norm_vec, scale_vec);
    ...
    col_max(max_vec, att_block[0], max_vec_prev);
    float max_scale_diff_ep3 = get_max_diff(max_vec_prev, max_vec);
    if (max_scale_diff_ep3 * TEMPERATURE_SCALE >= -RESCALE_THRESHOLD) {
        copy(max_vec, max_vec_prev);
        sub_col(att_block[0], att_block[0], max_vec);
    } else {
        sub(scale_vec, max_vec_prev, max_vec);
    ...
    exp2_optimized(att_block[0].tiles[0][0], att_block[0].tiles[0][0], num_tiles - 2, num_tiles);
    ...
    exp2_optimized(att_block[0].tiles[1][0], att_block[0].tiles[1][0], num_tiles - 2, num_tiles);
    mul(norm_vec, norm_vec, scale_vec);
    ...
    sub_col(att_block[1], att_block[1], max_vec);
    exp2_optimized(att_block[1].tiles[0][0], att_block[1].tiles[0][0], num_tiles - 1, num_tiles);
    sched_barrier_pairs<10, 5, 10>();
    sched_barrier_exp_pairs<6, 3, 10>();
    __builtin_amdgcn_sched_barrier(0);

    exp2_optimized(att_block[1].tiles[1][0], att_block[1].tiles[1][0], num_tiles - 1, num_tiles);
    mul(norm_vec, norm_vec, scale_vec);
